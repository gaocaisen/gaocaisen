...
<script type="text/javascript">
	...
	function totalPages(){
		$.ajax({
	        type: "post",//请求类型
	        url: "article/getTotalRowsFromArticle.do",//请求的url
	...
	/* 展示所有文章信息 */
	function listArticles(count){
		$("#dept_table").empty();
		$.ajax({
	        type: "post",//请求类型
	        url: "article/listArticle.do",//请求的url
	        data:{
	        	count:count
	        },
	...
	function addArticle(){
		...
		//修改新增的路径
		$("#windom_form").attr("action","article/addArticle.do");
		//回填数据
		$.ajax({
	        type: "post",//请求类型
	...
	function changeArticle(aid){
		...
		$("#windom_form").attr("action","article/changeArticle.do");
		$("#hidden_aid").val(aid);
		//进行标题的回填和栏目的回填
		$.ajax({
	        type: "post",//请求类型
	...
	function add(){
		$.ajax({
	        type: "post",//请求类型
	        url: "article/addArticle.do",//请求的url
	        data: $("#windom_form").serialize(), 
	        dataType: "text",//ajax接口（请求url）返回的数据类型
	        success: function (list) {//data：返回数据（json对象）
	        	alert("1");
